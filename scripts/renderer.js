  ...
  window.BlogOverlayRenderer = {
    ...
    _createCollectionPostCategoriesLine: function(post, siteAccent, categoryFilterUiEnabled, opts) {
      ...
      function appendCategoryEl(catName) {
        ...
        self._applyCategoryLabelStyle(el, { onImage: onDark });
        if (categoryFilterUiEnabled) {
      ...
      if (headingLinks.length > 0) {
        ...
        for (var hj = 0; hj < headingLinks.length; hj++) {
          ...
          if (tocStyle === 'bookmark') {
            link.style.backgroundColor = isActive ? 'var(--bb-accent-10, rgba(91,79,232,0.1))' : '';
            link.style.borderLeft = isActive ? '3px solid var(--bb-accent, #5B4FE8)' : '3px solid transparent';
          }
    ...
    _ensureCollectionStylesheet: function() {
      ...
      var css =
        ...
        '#blog-overlay-list .blog-overlay-feature-header-stack{max-width:800px;margin-left:auto;margin-right:auto;}' +
        ...
        '#blog-overlay-list .blog-overlay-story-rule{width:100%;height:1px;background:rgba(255,255,255,0.2);border:none;margin:0;}' +
        '#blog-overlay-list .blog-overlay-single-post-header-zone--story{background:#000;padding:0;margin-bottom:24px;}' +
        '#blog-overlay-list .blog-overlay-story-header-row{gap:40px;align-items:stretch;margin-bottom:0;}' +
        '#blog-overlay-list .blog-overlay-story-featured-image{flex:0 0 58%;min-width:0;align-self:stretch;display:flex;margin:0;}' +
        '#blog-overlay-list .blog-overlay-story-featured-image>div{flex:1 1 auto;width:100%;height:100%;min-height:0;overflow:hidden;}' +
        '#blog-overlay-list .blog-overlay-story-featured-image img{width:100%;height:100%;object-fit:cover;display:block;}' +
        '#blog-overlay-list .blog-overlay-story-info-panel{background:transparent;padding:32px;border-radius:0;box-sizing:border-box;gap:0;}' +
        '#blog-overlay-list .blog-overlay-story-info-panel .blog-overlay-post-breadcrumbs{margin-bottom:24px;}' +
        '#blog-overlay-list .blog-overlay-story-info-panel .blog-overlay-post-header-categories{margin-bottom:18px;}' +
        ...
        '#blog-overlay-list .bb-toc-item{display:block;font-size:16px;color:var(--bb-muted,#888);border-left:2px solid transparent;padding:8px 12px;text-decoration:none;}' +
        '#blog-overlay-list .bb-toc-item.is-active{color:var(--bb-accent,#5B4FE8);border-left-color:var(--bb-accent,#5B4FE8);background:var(--bb-accent-10,rgba(91,79,232,0.1));font-weight:600;}' +
    ...
    _applyCategoryLabelStyle: function(el, opts) {
      ...
      if (opts.onImage) el.classList.add('bb-category-label--on-image');
    ...
    /** The Reporter post template: split header (image right), body row with right sidebar. */
    ...
    _renderStandardPostListIntoMain: function(mainEl, items, vs, placeholderMap, navbarOffset, ensureSinglePostHeaderInnerEl, headerZoneRef) {
      ...
      for (var j = 0; j < displayItemsForLoop.length; j++) {
        ...
        var reporterPostHeaderLayout = isSinglePost && phImagePos === 'rightOfInfo';
        ...
        if (fiShow && (imgUrl || showFiPlaceholder) && !singlePostFullBleedHero && !singlePostFullBleedStacked) {
          ...
          if (fiLayout === 'fullBleed') {
            ...
            fiWrap.style.alignSelf = storyPostLayout
              ? 'stretch'
          ...
          if (fiFixedAspectCrop || digestFeaturedViewportBleed) {
            ...
            if (storyPostLayout) fiInner.style.height = '100%';
          }
        ...
        if (isSinglePost && phShowBreadcrumbs) {
          ...
          bcNav.className = 'blog-overlay-post-breadcrumbs' + ((singlePostFullBleedHero || storyPostLayout) ? ' blog-overlay-post-breadcrumbs--on-dark' : '');
        ...
        if (postHeaderCategoryLayout) {
          ...
          var postHeaderCatsLine = self._createCollectionPostCategoriesLine(post, siteAccentForPostCats, false, {
            onDark: singlePostFullBleedHero || storyPostLayout
          });
          if (postHeaderCatsLine) {
            postHeaderCatsLine.classList.add('blog-overlay-post-header-categories');
            if (!storyPostLayout) postHeaderCatsLine.style.marginBottom = '12px';
        ...
        if (isSinglePost) {
          ...
          self._applyTitleStyle(titleEl, { size: 'post', onImage: singlePostFullBleedHero || storyPostLayout });
        ...
        if (phShowByline && postInfoWrap && !postHeaderAccentDividerLayout && !publisherPostLayout) {
          ...
          if (bylineText) {
            ...
            bylineEl.className = 'blog-overlay-post-deck' + (reporterPostHeaderLayout ? ' blog-overlay-post-deck--reporter' : '');
        ...
        if (metaParts.length > 0 && !mastheadPaywallGatedCard && !digestPaywallGatedCard && !newsroomPaywallMobile) {
          ...
          if (singlePostFullBleedHero || storyPostLayout) {
            self._applyMetaStyle(meta, { onImage: true, variant: isSinglePost ? 'post' : null });
            if (singlePostFullBleedHero) meta.style.textShadow = '0 1px 2px rgba(0,0,0,0.5)';
        ...
        if (postHeaderAccentDividerLayout && postInfoWrap) {
          var accentExcerptSource = self._plainTextFromBlogHtml(post.excerpt || post.body || '');
          var accentExcerptSentences = accentExcerptSource ? accentExcerptSource.match(/[^.!?]*[.!?]/g) : null;
          var accentExcerptText = accentExcerptSentences && accentExcerptSentences.length > 0 ? accentExcerptSentences[0].trim() : '';
          if (!accentExcerptText) accentExcerptText = self._truncateText(post.excerpt || post.body || '', 200);
          if (accentExcerptText) accentExcerptText = self._stripLeadingSquarespaceSectionMarkers(accentExcerptText);
          if (accentExcerptText) {
            var accentExcerptEl = document.createElement('p');
            accentExcerptEl.className = 'blog-overlay-post-deck' + (writerPostLayout ? ' blog-overlay-post-deck--writer' : ' blog-overlay-post-deck--on-dark');
            accentExcerptEl.textContent = accentExcerptText;
            postInfoWrap.appendChild(accentExcerptEl);
          }
        ...
        if (isSinglePost && postInfoWrap) {
          ...
          if (deck && (!singlePostFullBleedStacked || featurePostLayoutForCat) && !phShowByline && !postHeaderAccentDividerLayout && !publisherPostLayout) {
            var deckEl = document.createElement('p');
            var deckModifier = featurePostLayoutForCat ? ' blog-overlay-post-deck--feature' : (singlePostFullBleedHero ? ' blog-overlay-post-deck--on-dark' : '');
        ...
        if (isSinglePost && postBreadcrumbNav) {
          ...
          } else if (storyPostLayout && postInfoWrap) {
            postBreadcrumbNav.style.justifyContent = bcJustify;
    ...
    _renderContent: async function(items) {
      ...
      function ensureSinglePostHeaderInnerEl() {
        ...
        if (self._isStoryPostLayout(cfg)) {
          ...
          singlePostHeaderZoneEl.style.paddingBottom = '0';
          singlePostHeaderZoneEl.style.paddingLeft = '0';
          singlePostHeaderZoneEl.style.paddingRight = '0';
            ...
            link.onclick = function(e) {
              ...
              try {
                ...
                for (var li = 0; li < tocLinks.length; li++) {
                  ...
                  if (tocStyle !== 'numbered') other.style.fontWeight = active ? '700' : '';
                }
              } catch (e2) {}
          ...
          if (footerContentCfg && footerContentCfg.show) {
            ...
            if (fcModules.length > 0) {
              ...
              for (var fm = 0; fm < fcModules.length; fm++) {
                ...
                /** Prev/next navigation is Writer-only. */
              ...
              if (!featurePostLayout && footerEl.childNodes.length > 0) footerZoneEl.appendChild(footerEl);
            }
          }
