...
(function() {
  ...
  window.BlogOverlayRenderer = {
    ...
    _createCollectionPostCategoriesLine: function(post, siteAccent, categoryFilterUiEnabled, opts) {
      ...
      wrap.style.fontSize = compact ? '0.75rem' : '0.85rem';
    ...
    _createSidebarTopicBadgeList: function(values, filterKey, width, opts) {
      ...
      for (var i = 0; i < values.length; i++) {
        (function(val) {
          ...
          btn.style.border = '1px solid ' + (isActive ? accent : '#d0d0d0');
          btn.style.background = isActive ? accent : '#fff';
          btn.style.color = isActive ? '#fff' : '#1a1a1a';
          btn.style.transition = 'background 0.12s ease, color 0.12s ease, border-color 0.12s ease';
    ...
    _computeCollectionViewState: function(items) {
      ...
      var gridMobileNarrow = !isSinglePost && collectionLayout === 'grid' && self._isNarrowCollectionViewport();
      var gridColsEffective = gridMobileNarrow ? 2 : gridColsDigestOrGrid;
      ...
      try {
        ...
        var renderLogPayload = {
          ...
          paywallShowFooter: paywalledLoggedOut,
      ...
      if (usePagination && (paginationMode === 'infiniteScroll' ? totalFiltered > 0 : totalPages > 1)) {
        ...
        if (paginationMode === 'infiniteScroll') {
          ...
          if (self._infiniteScrollLoaded < totalFiltered) {
            ...
            loadMoreBtn.style.background = accentPag;
          ...
          var makePageBtn = function(pageNum, isCurrent) {
            ...
            btn.style.border = '1px solid ' + (isCurrent ? accentPag : '#ddd');
            btn.style.borderRadius = '6px';
            btn.style.background = isCurrent ? accentPag : 'white';
    ...
    _renderStandardPostListIntoMain: function(mainEl, items, vs, placeholderMap, navbarOffset, ensureSinglePostHeaderInnerEl, headerZoneRef) {
      ...
      /** Digest: featured hero image breaks out to viewport width on narrow viewports / preview phone */
      var digestMobileFullBleed = false;
      if (!isSinglePost && collectionLayout === 'digest') {
        try {
          var dVpNarrow = typeof window !== 'undefined' && (window.matchMedia ? window.matchMedia('(max-width: 767px)').matches : window.innerWidth <= 767);
          var dRootW = 0;
          if (self._root && self._root.getBoundingClientRect) {
            dRootW = Math.round(self._root.getBoundingClientRect().width) || 0;
          }
          if (!dRootW && self._root && self._root.clientWidth) dRootW = self._root.clientWidth;
          var dPreviewPhone = Boolean(self._previewMode && self.config && self.config.previewDevice === 'mobile');
          digestMobileFullBleed = dVpNarrow || (dRootW > 0 && dRootW <= 767) || dPreviewPhone;
        } catch (eDigMob) {
      ...
      for (var j = 0; j < displayItemsForLoop.length; j++) {
        ...
        if (isFeaturedInLayout && (collectionLayout === 'grid' || collectionLayout === 'digest')) {
          var featuredSpanCols = collectionLayout === 'grid' ? gridColsEffective : gridColsDigestOrGrid;
          article.style.gridColumn = '1 / span ' + featuredSpanCols;
        }
        if (!isSinglePost && collectionLayout === 'grid') {
        ...
        if (singlePostFullBleedStacked) {
          ...
          if (imgUrl) {
            ...
            stackImg.style.objectFit = fiFixedAspectCrop ? 'cover' : 'contain';
        ...
        if (fiShow && (imgUrl || showFiPlaceholder) && !singlePostFullBleedHero && !singlePostFullBleedStacked) {
          ...
          if (fiLayout === 'fullBleed') {
            ...
            if (collectionLayout === 'listRows' && listRowsMobileCompact) {
              fiWrap.style.flex = '0 0 88px';
              fiWrap.style.width = '88px';
              fiWrap.style.maxWidth = '88px';
            } else {
              fiWrap.style.flex = '0 0 ' + fiImageWidth + '%';
            }
            fiWrap.style.minWidth = '0';
            fiWrap.style.alignSelf = 'flex-start';
          ...
          if (imgUrl) {
            ...
            img.style.objectFit = (fiFixedAspectCrop || digestFeaturedViewportBleed) ? 'cover' : 'contain';
        ...
        var postCategoriesLine = (!isSinglePost && (collectionLayout === 'listRows' || collectionLayout === 'grid' || collectionLayout === 'digest'))
          ? self._createCollectionPostCategoriesLine(post, siteAccentForPostCats, categoryFilterUiEnabled, {
            compact: collectionLayout === 'listRows' && listRowsMobileCompact
          })
          : null;
        ...
        if (isFeaturedInLayout) {
          ...
          featuredHeadlineStack.style.gap = (collectionLayout === 'listRows' && listRowsMobileCompact) ? '4px' : '6px';
          ...
          featuredBadge.style.alignSelf = 'flex-start';
          featuredBadge.style.width = 'fit-content';
          featuredBadge.style.maxWidth = '100%';
          featuredBadge.style.background = siteAccentForPostCats;
        ...
        if (isSinglePost && postInfoWrap) {
          var deckText = self._plainTextFromBlogHtml(post.excerpt || post.body || '');
    ...
    _renderShowcasePostsIntoMain: function(mainEl, items, vs, placeholderMap, navbarOffset) {
      ...
      var categoryFilterUiEnabled = vs.categoryFilterUiEnabled;
        ...
        card.style.gap = '0';
        card.style.marginTop = j === 0 ? '0' : '7.5%';
        ...
        if (!isSinglePost && faCfg && faCfg.show && faCfg.position === 'inLayout' && featuredPost) {
          ...
          if (post === featuredPost || (shFpK && shPk === shFpK)) {
            ...
            shBadge.style.width = 'fit-content';
    ...
    _refreshCollectionPostsOnly: async function(items) {
      ...
      if (collectionLayout === 'grid' || collectionLayout === 'digest') {
        ...
        var gridColsForShell = collectionLayout === 'grid' ? gridColsEffective : gridColsDigestOrGrid;
        mainEl.style.gridTemplateColumns = 'repeat(' + gridColsForShell + ', 1fr)';
    ...
    _renderContent: async function(items) {
      ...
      if (isSinglePost) {
        ...
        var gridColsForMain = collectionLayout === 'grid' ? gridColsEffective : gridColsDigestOrGrid;
        main.style.gridTemplateColumns = 'repeat(' + gridColsForMain + ', 1fr)';
        main.style.gap = gridMobileNarrow && collectionLayout === 'grid' ? '16px' : '24px';
      ...
      if (!isSinglePost) {
        ...
        headerModulesHostEl.style.alignItems = 'center';
        headerModulesHostEl.style.gap = '12px 16px';
      ...
      if (faCfg && faCfg.position === 'header' && featuredPost) {
          ...
          heroBadge.style.background = self._getSiteAccentColor();
          ...
          if (mastheadHeroMobile) {
            ...
            heroBadge.style.top = '16px';
            heroBadge.style.left = '16px';
            heroBadge.style.zIndex = '3';
            heroBadge.style.marginBottom = '0';
      ...
      function createSidebarSection(headerText, content, styled) {
        ...
        if (headerText === 'Topics' || headerText === 'Tags') {
          header.style.fontWeight = '700';
          header.style.color = '#111';
        }
        section.appendChild(header);
        var bar = document.createElement('div');
        bar.style.height = (headerText === 'Topics' || headerText === 'Tags') ? '2px' : '1px';
        bar.style.background = (headerText === 'Topics' || headerText === 'Tags') ? '#111' : '#e5e4e0';
            ...
            link.onclick = function(e) {
              ...
              try {
                ...
                for (var li = 0; li < tocLinks.length; li++) {
                  ...
                  other.style.fontWeight = active ? '700' : '';
                }
              } catch (e2) {}
      ...
      function createEmailCaptureForm(ecCfg, width, hideHeader, placement) {
        ...
        if (placement === 'footer') {
          ...
          btnF.style.background = siteAccentUi;
      ...
      if (collectionLayout === 'editorial') {
        ...
        var makeEditorialCard = function(p, isLarge, mobilePairCard) {
          ...
          if (!isSinglePost && faCfg && faCfg.show && faCfg.position === 'inLayout' && featuredPost) {
            ...
            if (p === featuredPost || (edFpK && edPk === edFpK)) {
              var edBadge = document.createElement('div');
              edBadge.textContent = 'FEATURED';
              edBadge.style.position = 'absolute';
              edBadge.style.top = '14px';
              edBadge.style.left = '14px';
              edBadge.style.zIndex = '4';
              edBadge.style.background = self._getSiteAccentColor();
              edBadge.style.color = '#fff';
              edBadge.style.fontSize = '9px';
              edBadge.style.fontWeight = '700';
              edBadge.style.letterSpacing = '1.5px';
              edBadge.style.textTransform = 'uppercase';
              edBadge.style.padding = '3px 8px';
              edBadge.style.borderRadius = '2px';
        ...
        this._renderShowcasePostsIntoMain(main, items, vs, placeholderMap, navbarOffset);
          ...
          function applySidebarResponsiveLayout() {
            var leftHas = leftSidebarEl.childNodes.length > 0;
            var rightHas = rightSidebarEl.childNodes.length > 0;
            if (!leftHas && !rightHas) {
              mainRowEl.style.flexDirection = 'row';
              mainRowEl.style.alignItems = 'flex-start';
              mainRowEl.style.gap = BB_SIDEBAR_LAYOUT_GAP + 'px';
            ...
            if (stack) {
              ...
              if (leftHas) {
                leftSidebarEl.style.order = '1';
                leftSidebarEl.style.width = '100%';
                leftSidebarEl.style.maxWidth = '100%';
                leftSidebarEl.style.flexShrink = '1';
              ...
              if (rightHas) {
                ...
                rightSidebarEl.style.maxWidth = '100%';
                rightSidebarEl.style.flexShrink = '1';
              ...
              if (leftHas) {
                leftSidebarEl.style.order = '';
                leftSidebarEl.style.width = leftSidebarWidth + 'px';
                leftSidebarEl.style.maxWidth = '';
                leftSidebarEl.style.flexShrink = '0';
                if (leftSticky) {
                  leftSidebarEl.style.position = 'sticky';
                  leftSidebarEl.style.top = (navbarOffset + 16 + collectionHeaderStickyOffset) + 'px';
              ...
              if (rightHas) {
                rightSidebarEl.style.order = '';
                rightSidebarEl.style.width = rightSidebarWidth + 'px';
                rightSidebarEl.style.maxWidth = '';
                rightSidebarEl.style.flexShrink = '0';
                if (rightSticky) {
                  rightSidebarEl.style.position = 'sticky';
                  rightSidebarEl.style.top = (navbarOffset + 16 + collectionHeaderStickyOffset) + 'px';
          ...
          if (!isSinglePost) {
            this._lastCollectionShellKey = collectionLayout + '|' + (collectionLayout === 'grid' ? gridColsEffective : gridColsDigestOrGrid);
          } else {
