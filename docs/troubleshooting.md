...
If your API key is valid but verification isn't working, check that the commenter's email actually exists in your Squarespace member list. Verification looks up the email against your Profiles API. If the email isn't found and anonymous comments are off, the reader sees a modal and the comment is not posted. The same message also stays under the comment button — accent color, italic, regular weight, slightly smaller than the button — and the button stays visible. It remains until a later attempt verifies and the comment posts. Clicking the comment button again opens the email confirmation modal instead of resubmitting the failed address. If anonymous comments are also on, they are asked to confirm posting as a guest — BetterBlog does not silently store an unverified comment as anonymous. The inline message stays until a comment posts, and another click on the comment button opens the email modal again.
